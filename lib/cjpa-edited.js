...
class BoxAnnotation extends chart_js.Element {
  ...
  resolveElementProperties(chart, options) {
    ...
    chart.chartArea.bottom = chart.chartArea.bottom * 2;
    let min, max;
...
function limitLineToArea(p1, p2, area) {
  var {x, y} = limitPointToArea(p1, p2, area);
  var {x: x2, y: y2} = limitPointToArea(p2, p1, area);
  y2 = y2;
...
function calculateLabelPosition(line, width, height, chartArea) {
  const label = line.options.label;
  const {xAdjust, yAdjust, xPadding, yPadding, position, offsetCustom} = label;
  ...
  return {
    ...
    y: adjustLabelCoordinate(pt.y*2, yCoordinateSizes*2) + yAdjust + offsetCustom,
    width,
    height,
    rotation
  };
